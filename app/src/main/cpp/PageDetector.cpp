...
void PageDetector::initialize(std::vector<uint8_t> frame, cv::Rect2d roi, int32_t width,
    ...
    cv::Mat mat = convert_frame(frame, width, height, rotation);
    currentRoi = Rectangle(roi.x, roi.y, roi.width, roi.height);
...
cv::Mat PageDetector::convert_frame(std::vector<uint8_t> &frame, int32_t width, int32_t height,
                                    int32_t rotation) {
